...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRect(check_bb.Min + ImVec2(2, 2), check_bb.Max - ImVec2(2, 2), IM_COL32(240, 240, 240, 180), 4.f);

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2, 2), check_bb.Max - ImVec2(2, 2), GetColorU32(ImVec4(255 / 255.f, 1 / 255.f, 1 / 255.f, *alpha)), 4.f);

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 3.5f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), GetColorU32(ImVec4(1.f, 1.f, 1.f, *alpha)), square_sz - pad * 2.0f);
  

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 1.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), IM_COL32(45, 45, 45, 255), style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, IM_COL32(45, 45, 45, 255), 1.5F);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
    window->DrawList->AddRect(bb.Min, bb.Max, IM_COL32(116, 60, 200, 255), style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Min.x /*+ style.ItemInnerSpacing.x*/, bb.Min.y - style.FramePadding.y - label_size.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + style.FramePadding.y * 2.0f), window->DC.CursorPos + ImVec2(w, label_size.y * 2 + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0.f, label_size.y > 0.0f ? style.FramePadding.y + label_size.y * 2.f : 0.f));
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(0, 5), IM_COL32(45, 45, 45, 255), true, 4.f);


    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
       RenderFrame(frame_bb.Min + ImVec2(0, 4), grab_bb.Max - ImVec2(0, 4), IM_COL32(255, 0, 0, 255), true, 4.f);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5, 5), 8, IM_COL32(1, 1, 1, 255), 360);
    window->DrawList->AddCircle(grab_bb.Min + ImVec2(5, 5), 8, IM_COL32(255, 0, 0, 255), 360, 2.F);
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
 
    RenderText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x , total_bb.Min.y + style.FramePadding.y - 19.5f), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x /*+ style.ItemInnerSpacing.x*/, frame_bb.Min.y - style.FramePadding.y - label_size.y), label);
